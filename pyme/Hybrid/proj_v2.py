...
# This algorithm, the projection puts a filter on the observations, that it has to see atleast 10 observataions to consider it a state.
...
def project_to_Hybrid(X,w,stoc_vector):
	...
	new_N = len(Unique_X)
	p_1 = np.array(P_1)
	outlayers = np.zeros((new_N,),dtype=np.bool)
	output_X = np.zeros((X.shape[0],new_N))
	
	#print("['Caution']  more than 2 times ")
	for i,x in enumerate(Unique_X):
		#pdb.set_trace()
		output_X[stoc_vector,i] = x
		#p_1[i] = P_1[i]
		if len(Y_for_X[i]) > 0: # we only consider simulations where there were more than 10 observations.
	...
	keeps = np.invert(outlayers)
	
	return output_X[:,keeps], p_1[keeps]
