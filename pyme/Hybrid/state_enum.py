...
class StateEnum(object):
    ...
    """
    def __init__(self, initial_states, stoc_vec = None):
        """
    ...
    def update_ordering(self):
        ...
        if self.size > 0:
            ...
            self.ordered_full_dom = self.unordered_full_dom[:,order]
        else:
            self.ordered_states = self.unordered_states
            self.ordered_full_dom = self.unordered_full_dom
    ...
    def reinitialise(self, initial_states):
        ...
        self.unordered_states = initial_states
    ...
    def indices(self, states):
        ...
        return member_index[unique_inverse]
            ...
            #unique_states, unique_inverse = lexarrayset.unique(states[self.stoc_vec,:],return_inverse=True)
            #members = lexarrayset.member(self.ordered_states[self.stoc_vec,:], unique_states)
            #member_index = numpy.array(self.index[members] + self.offset)
            #return member_index[unique_inverse]
